...
void main() {
  ...
  group('findNavBarInfo', () {
    test('finds BottomNavigationBar and counts children', () {
      final tree = {
        ...
        'children': <dynamic>[
          ...
          },
          {
            ...
            'widgetRuntimeType': 'BottomNavigationBar',
            'children': <dynamic>[
              {
                'description': 'BottomNavigationBarItem',
                'widgetRuntimeType': 'BottomNavigationBarItem',
                'children': <dynamic>[],
              },
              {
                'description': 'BottomNavigationBarItem',
      ...
      final info = FlutterInspector.findNavBarInfo(tree);
      expect(info, isNotNull);
      expect(info!.type, equals('BottomNavigationBar'));
      expect(info.tabCount, equals(3));
    });

    test('finds NavigationBar with NavigationDestination children', () {
      final tree = {
        'description': 'Scaffold',
        'widgetRuntimeType': 'Scaffold',
        'children': <dynamic>[
          {
            ...
            'widgetRuntimeType': 'NavigationBar',
            'children': <dynamic>[
              {
                'description': 'NavigationDestination',
                'widgetRuntimeType': 'NavigationDestination',
                'children': <dynamic>[],
              },
              {
                'description': 'NavigationDestination',
      ...
      final info = FlutterInspector.findNavBarInfo(tree);
      expect(info, isNotNull);
      expect(info!.type, equals('NavigationBar'));
      expect(info.tabCount, equals(2));
    });

    test('falls back to child count when no recognized tab types', () {
      final tree = {
        'description': 'BottomNavigationBar',
        'widgetRuntimeType': 'BottomNavigationBar',
        'children': <dynamic>[
          {'description': 'Icon', 'widgetRuntimeType': 'Icon', 'children': <dynamic>[]},
          {'description': 'Icon', 'widgetRuntimeType': 'Icon', 'children': <dynamic>[]},
          {'description': 'Icon', 'widgetRuntimeType': 'Icon', 'children': <dynamic>[]},
          {'description': 'Icon', 'widgetRuntimeType': 'Icon', 'children': <dynamic>[]},
        ],
      ...
      final info = FlutterInspector.findNavBarInfo(tree);
      expect(info, isNotNull);
      expect(info!.tabCount, equals(4));
    });

    test('returns null when no navigation bar found', () {
      final tree = {
        'description': 'Scaffold',
        'widgetRuntimeType': 'Scaffold',
        'children': <dynamic>[
      ...
      expect(FlutterInspector.findNavBarInfo(tree), isNull);
    });
  });
