...
class FlutterInspector {
  ...
  /// Extract bounds from a node's own description (not its renderObject).
  ///
  /// Used when getDetailsSubtree returns a render object node directly
  /// (e.g. when we used the renderObject's valueId instead of the widget's).
  static Bounds _extractBoundsFromDescription(Map<String, dynamic> node) {
  ...
  /// Recursively walk a detail subtree looking for keyed widgets that
  /// need bounds.
  void _extractKeyedChildBounds(
    Map<String, dynamic> node,
    List<WidgetStyle> widgets,
    ...
    for (final child in children) {
      ...
      // Check if this detail node corresponds to a keyed widget
      final key = _extractKey(child);
      final desc = child['description'] as String? ?? '';

      // Diagnostic: log detail tree nodes that look like they might
      // have a key but _extractKey couldn't parse it.
      if (key == null && keyToIndex.isNotEmpty) {
        if (desc.contains('[<') || desc.contains('Key(')) {
          stderr.writeln(
            '[saccadic:diag] Detail child has key-like desc but '
            '_extractKey returned null: "$desc"',
          );
          stderr.writeln(
            '[saccadic:diag]   child keys: ${child.keys.toList()..sort()}',
          );
          final props = child['properties'] as List?;
          if (props != null) {
            for (final p in props) {
              if (p is Map && p['name'] == 'key') {
                stderr.writeln(
                  '[saccadic:diag]   key property: ${p['description']}',
                );
              }
            }
          }
      ...
      if (key != null && keyToIndex.containsKey(key)) {
        var bounds = _extractBounds(child);
        if (bounds.width == 0 && bounds.height == 0) {
          bounds = _extractBoundsFromDescription(child);
        }
        if (bounds.width > 0 && bounds.height > 0) {
          final idx = keyToIndex[key]!;
          _updateWidgetBounds(widgets, idx, bounds);
          ...
          // Diagnostic: we found the key but couldn't get bounds
          stderr.writeln(
            '[saccadic:diag] Found key "$key" in detail tree but bounds '
            'are still 0. desc: "$desc"',
          );
          stderr.writeln(
            '[saccadic:diag]   child keys: ${child.keys.toList()..sort()}',
          );
          ...
          if (ro is Map) {
            stderr.writeln(
              '[saccadic:diag]   renderObject.desc: ${ro['description']}',
            );
          } else {
            stderr.writeln(
              '[saccadic:diag]   NO renderObject on detail child',
            );
          }
        }
      }
      ...
      // Recurse deeper
      _extractKeyedChildBounds(child, widgets, keyToIndex);
    }
  }

  /// Walk the summary tree to collect valueIds mapped to widget list indices.
  void _collectValueIds(
    Map<String, dynamic> node,
    List<WidgetStyle> widgets,
    ...
    if (valueId != null) {
      // Find the widget at the current position in the flat list.
      // The walk order matches _walkWidgetTree, so index tracks position.
      final description = node['description'] as String? ?? '';
      final widgetType =
          node['widgetRuntimeType'] as String? ?? description;
      ...
      // Diagnostic: log keyed widgets that have no valueId so we can
      // understand what data IS available for them in the summary tree.
      final key = _extractKey(node);
      if (key != null) {
        final type = node['widgetRuntimeType'] ?? node['description'];
        final nodeKeys = node.keys.toList()..sort();
        stderr.writeln(
          '[saccadic:diag] Keyed widget "$key" ($type) has NO valueId. '
          'Available keys: $nodeKeys',
        );
        final ro = node['renderObject'];
        if (ro is Map) {
          stderr.writeln(
            '[saccadic:diag]   renderObject keys: ${ro.keys.toList()}, '
            'valueId: ${ro['valueId']}',
          );
        } else {
          stderr.writeln(
            '[saccadic:diag]   NO renderObject in summary tree node',
          );
        }
      }
    }
    ...
    if (children != null) {
      for (final child in children) {
        if (child is Map<String, dynamic>) {
          _collectValueIds(child, widgets, valueIdToIndices, startIndex);
        }
      }
    }
  }
